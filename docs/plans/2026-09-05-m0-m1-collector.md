...
- **Refresh budget, 2026-09-06:** run 50's real numbers (above) said the tick finished, not that it was affordable to run forever — refreshing a pool costs ~10 Blockfrost calls on these shared-address venues, so refreshing all 102 discovered pools every 5 minutes (288 ticks/day) is `102 x 10 x 288 ≈ 294k` calls/day against a 50k/day free quota. Candles already only read the deepest pool per token per tick (`buildCandles` in `@ctb/candles`), and the deepest pool per token is on Minswap v2 for 19/20 tokens (AGIX is the one exception) — Minswap v1's 14 pools cost ~9,600 discovery calls/day for pools that are otherwise never the deepest. Two levers, both in `feat/refresh-budget`: `DexterPoolSource`'s `refreshPolicy: 'deepest' | 'all'` (class default `'all'`, unchanged for any caller that doesn't opt in) prunes the known set to one pool per base token — largest ADA reserve, ties broken by smallest identifier — right after `discover()` and before `refresh()` spends a call (a discovery tick still writes every pool it found to `pool_snapshots`); and Minswap v1 is now `enabledByDefault: false` in `venues.ts` (still requestable via `COLLECT_VENUES=Minswap,...`). Resulting CLI defaults (`packages/cli/src/config.ts`): `COLLECT_INTERVAL_SECONDS=600` (10 minutes, down from 300) and `COLLECT_REFRESH=deepest` (~20 pools) — refresh becomes `~20 x 10 x 144 ≈ 28,800` calls/day plus discovery's own daily cost, comfortably under quota. `DEFAULT_VENUES` is now exactly six venues: `MinswapV2, SundaeSwapV1, SundaeSwapV3, MuesliSwap, WingRiders, WingRidersV2`.
