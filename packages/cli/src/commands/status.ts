...
import { effectiveTickIntervalSec, heartbeatAgeCell, isHeartbeatStale, missingTicksCell } from '@ctb/reports';
...
export async function statusCommand(log: Logger, args: string[] = []): Promise<void> {
  ...
  try {
    ...
    // The interval a ROW is written at, which is the focus interval when tiered sampling is on —
    // not cfg.intervalSec, the candle interval. Passing the latter is what printed -191.
    const tickIntervalSec = effectiveTickIntervalSec(cfg);
    const cadence = await repo.missingTicksApprox(tickIntervalSec);
    ...
    if (digest) {
      ...
      for (const line of digestLines(await repo.digestInput(tickIntervalSec, [...cfg.venues], now), now)) console.log(line);
    ...
    console.log(`ticks missing in last 24h (approx): ${missingTicksCell(cadence)}`);
