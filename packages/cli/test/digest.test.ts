...
describe('digestLines', () => {
  it('reads a healthy mid-day collector as OK with a day projection from the UTC-midnight elapsed time', () => {
    ...
    expect(lines[3]).toBe('venues found since the last discovery: all 2 configured');
  ...
  it('tells a venue LOST at discovery apart from one PRUNED by the deepest policy, and counts tokens left without a pool (the live run lost MinswapV2 at 00:20 UTC)', () => {
    ...
    const lines = digestLines({ ...base, venuesConfigured: ['MinswapV2', 'MuesliSwap', 'SundaeSwapV3'], venuesSinceLastDiscovery: ['MuesliSwap', 'SundaeSwapV3'], venuesInLastTick: ['SundaeSwapV3'], tokensCoveredInLastTick: 16 }, now);
    expect(lines[3]).toBe('venues LOST since the last discovery: MinswapV2 (configured: MinswapV2, MuesliSwap, SundaeSwapV3) — retried every tick until they return; check the run rows\' errors if it persists');
