...
arithmetic, and no coverage arithmetic of its own. A guard test greps the
package for `returnPct`, `maxDrawdown`, `feesLovelace` and `/ 1_000_000`
assignments and fails on any that is not an import. The functions it needs
that live in `packages/cli` today (`digestLines`, the doctor checks, the
compare and grid row builders, `adaStr`) move to a new `@ctb/reports`
package in M4a so neither the CLI nor the dashboard imports the other.
...
return, max drawdown, fills / intents, warnings count, REHEARSAL. Return and
drawdown come from `runs.summary` for backtests and from `summarizeRun` over
persisted rows for paper runs, with the basis shown, as `compareRunRows` does.
Checkboxes and a "compare selected" button that only builds the `/compare`
URL.
...
the headline table, the equity chart for paper runs (all persisted points),
the reject-reasons table, and the orders table with the same columns as the
CLI, paginated at 200 rows. A backtest shows "equity is not persisted for
backtest runs" where the chart would be.
