...
export const VENUE_COSTS: Record<DexName, VenueCosts> = {
  ...
  SundaeSwapV3: { batcherFeeLovelace: 1_280_000n, networkFeeLovelace: NETWORK, basis: 'measured', source: 'MEASURED ON CHAIN 2026-09-16: six live mainnet V3 orders across FOUR distinct pools all carry protocolFee 1280000 in top-level datum field 2; one of them totals exactly 3.28 ADA, which is this fee plus the 2 ADA returned deposit and nothing else. NOTE the fee is a PER-POOL datum value (sundaeswap-v3.js:77 reads parameters.ProtocolFee, falling back to protocolFeeDefault), so read the pool before trading it. SundaeV3.pdf 4.4.3 documents a 0.5-1.0 range that neither the library nor the chain uses. Method and raw evidence: docs/ops/2026-09-16-sundaeswap-v3-protocol-fee.md. Pinned by dexterWritesTheBatcherFee.guard.test.ts', readAt: '2026-09-16' },
